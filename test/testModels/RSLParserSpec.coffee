(->
    ...
    describe "RSLParser", ->
        ...
        describe "execute method", ->
            ...
            describe "One shot instruction", ->
                dt = dt_secondPass = dt_branch = dt_branch_secondPass = null

                beforeEach ->
                    dt = new DataTable
                    dt_secondPass = new DataTable
                    dt_secondPass.B3 = {0: {0:true}}
                    dt_branch = new DataTable
                    dt_branch.addBranch()
                    dt_branch_secondPass = new DataTable
                    dt_branch_secondPass.addBranch()
                    dt_branch_secondPass.B3 = {0: {0:true}}

                it "sets its address true", ->
                    expect(RSLParser.execute("OSR,B3:0/0", dt).B3[0][0]).toBe true

                it "leaves the rung open on the first pass", ->
                    expect(RSLParser.execute("OSR,B3:0/0", dt).rungOpen).toBe true

                it "closes the rung on the second pass", ->
                    expect(RSLParser.execute("OSR,B3:0/0", dt_secondPass).rungOpen).toBe false

                it "leaves branch open on the first pass", ->
                    expect(RSLParser.execute("OSR,B3:0/0", dt_branch).branches[0].topLine).toBe true
        ...
        describe "runRung method", ->
            ...
            describe "one-shot rungs", ->

                rungText = "SOR,0 XIC,I:1/0 OSR,B3:0/0 OTE,O:2/0"

                dt = dt_secondPass = dt_thirdPass = null

                beforeEach ->
                    dt = new DataTable true
                    dt_secondPass = new DataTable true
                    dt_secondPass.B3 = {0: {0:true}}
                    dt_thirdPass = new DataTable false
                    dt_thirdPass.B3 = {0: {0:true}}

                it "acts as a short on the first pass", ->
                    expect(RSLParser.runRung(rungText, dt).O[2][0]).toBe true

                it "disables the rung on the second pass", ->
                    expect(RSLParser.runRung(rungText, dt_secondPass).O[2][0]).toBe false

                it "re-enables rung after input goes false", ->
                    expect(RSLParser.runRung(rungText, dt_thirdPass).B3[0][0]).toBe false
